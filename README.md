...
There used to be a second delivery — a single self-contained
`dist/C182_FlightPlanner.html` opened by double-clicking. It was removed at
v16.45: it went unused, and it was never the better offline option anyway
(a `file://` page cannot register a service worker, so it could not cache a
single chart tile). Keeping it also forced the 366 KB AIP dataset to be inlined
into the page, because `file://` forbids `fetch()`.
...
package.json) before anything reaches `site/`.

On Windows you can skip the command line entirely: `serve.cmd` builds the site,
serves it, and opens the browser.
...
Nothing built is committed — `site/` is regenerated by `npm run build` and by
CI before every deploy. The bundle is still a **classic** script rather than ES
modules, but that is no longer about `file://`: the page script is a classic
script whose inline `on*=` handlers need the bundle's functions as globals, and
a `type="module"` script is deferred, so it would run *after* the page script.
